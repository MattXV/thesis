...
\subsection{Acoustic Hear-Through Displays and Context-Aware Dereverberation}
...
\acrshortpl{naf} learn using joint acoustic/visual representations to construct a local geometric grid, that can then infer acoustic energy transfer from emitter to receiver by querying the radiance field.
%how does the neural acoustic field learn?
...
Even with radiance fields, the interface between the game engine and the \acrshort{dsp} engine would be maintained valid. The game engine would query the radiance field based on the current source and listener position.
