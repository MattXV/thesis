\chapter{Geometrical Acoustics Rendering Pipelines for Augmented Acoustics}\label{ch:acoustic-rendering} % If you're changing this, update Section 1.6
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{ir-components}
        \caption{\acrshort{ir} components}
    \end{subfigure}%
    ~ 
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{rir-summed}
...
\subsection{Preliminary Evaluation Using Learned Similarity}
...
\section{Ray Tracing-Based Rendering Pipeline}\label{sec:ga-based-pipeline}
...
\begin{itemize}
    ...
    \item leveraging fast rendering and processing techniques that can be optimised for wearable computing platforms such as \acrshortpl{hmd}.
\end{itemize}
...
A \acrfull{bvh} or \acrfull{bsp} is often employed for handling scene geometry in acoustic simulations \citep{schroder2011physically}. \acrshortpl{bvh} organise the scene geometry into a tree structure of nested volumes, typically using \acrfullpl{aabb}. Each node in the hierarchy contains a bounding volume that encompasses the scene's triangles. This hierarchical approach allows for rapid traversal and collision detection, significantly reducing the computational load when determining sound interactions within complex scenes.\par
...
As illustrated in Chapter~\ref{ch:lit-review}, deep learning-based approaches have not seen as much progress as tree structures due to their recent nature; \acrshortpl{bvh} have become fast and feasible for interactive applications \citep{kopta2012fast}.\par
The ray tracing-based renderer design implements a \acrfull{bvh} as a geometry handling system, allowing the dynamic indexing and searching of a given complex scene. The \acrshort{bvh} implementation constructs a binary tree of \acrshortpl{aabb} encapsulating triangles of the mesh representing the \acrshort{ve} and any entity interacting with the acoustic environment, e.g. a character representing a listener. 
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Function{$RandomOnUnitSphere$}{}\Comment{Random direction vector, uniformly sampled on unit sphere}
        \State{} $u_{\theta} \backsim U(0, 1)$
        \State{} $u_{\varphi} \backsim U(0, 1)$ 
        \State{} $\theta \gets 2 \pi u_{\theta}$\Comment{Uniform random between $0$ and $2\pi$}
        \State{} $\varphi \gets \arccos(2 u_{\varphi} - 1)$\Comment{Uniform random between $0$ and $\pi$}  
        \State{} $x \gets \sin(\varphi) \cos(\theta)$\Comment{Spherical to cartesian coordinate conversion} 
...
\input{air-absorption}

\subsubsection{Shadow Paths}
Diffraction simulations, as demonstrated by diffraction modelling using the Uniform Theory of Diffraction (UTD) \cite{tsingos2001modeling}, use reflecting geometry and edges as sources of new diffracted rays, complementing specular reflections. Similarly, the ray tracer considers the possible connections between intersections at intermediate nodes of propagation paths and the receiver volume, see Figure~\ref{fig:ray-tracing-diagram}. Such shadow paths contribute to the modelling of the energy decay and exploit computations involved in geometry searches and occlusion tests.\par
...
\end{equation}
The use of Poisson distributions allows the generation of perceptually valid \acrshortpl{rir} thanks to the layer of stochastic statistic modelling added to the process of constructing the response. This layer allows the Dirac-Delta series to be modelled with an approximated energy decay function, obtained via ray tracing. Without the distributions of Dirac-Delta pulses, more rays are needed to construct the final response.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{rir-processing}
    \caption[Ray tracing-based frequency-dependent Impulse Response generation]{Visualisation of frequency-dependent \acrshortpl{rir}, represented as energy logging resulting from generated propagation paths for a given source-receiver computation of Algorithm~\ref{alg:rt-main}. Poisson distribution calculated based on the environment dimensions obtained from the generated \acrshort{bvh} are then used to generate bipolar impulse responses.}\label{fig:rir-freqdep-poisson}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption[Ray tracing-based monoaural Impulse Response generation]{Time and frequency-domain representations left and right subfigures, respectively, of frequency-dependent \acrshortpl{rir} combined into a broadband monoaural response. }
...
\end{equation}
determining frequency-dependent $h$ functions that can construct a monoaural \acrshort{rir} by filtering each frequency component $h_f$ and summing their aggregate to a broadband response, see Figure~\ref{fig:rir-freqdep-monoaural}. A filter bank determines the contribution of each $h_f$ towards its specific portion of the spectrum, dictated by the spectrum partitioning of the absorption coefficients used; it is composed of \acrfull{fir} filters applied to the sequences. The filter bank is constructed using low-pass window-sinc filters for efficient computation against attenuation factors. High and band-pass filters are obtained via spectral inversions of window-sinc low-pass filters. Using \cite{smith1997scientist}'s implementation, the $h$ function is calculated for a low-pass $lpf$ filter for a digital sequence of $M$ points with sampling frequency $F_s$:
\begin{equation}
...
The acoustic rendering pipeline is deployed on the scene, defining two $0.5m$ radius spheres as source and receiver volumes and emitting $3*10^5$ rays, allowing $4$ orders of reflections. \acrshortpl{rir} are constructed in about $60s$ with an unoptimised and unparallelised implementation running on the Unity game engine scripting runtime. The same procedure described in the previous Section is repeated. \par

\subsection{Results}
\input{rt-metrics}

\input{rt-metrics-stats}

\input{g-metrics}

\acrshortpl{rir} obtained from both real and simulated soundfields are reported in Figures~\ref{fig:rir-comparisons-grid-a}, \ref{fig:rir-comparisons-grid-a2}, \ref{fig:rir-comparisons-grid-b}, \ref{fig:rir-comparisons-grid-b2}; these plots show Simulated and Measured \acrshortpl{rir} in the first and second columns, respectively, reporting comparisons of decay curves obtained from smoothed analytic signals from the absolute value of a Hilbert transform of the respective \acrshort{rir}. The Measured \acrshortpl{rir} have around $45dB$ of signal-to-noise ratio, and the Simulated ones are over $120dB$. \par
