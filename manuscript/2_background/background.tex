...
Amongst popular techniques for sound propagation are \acrfull{fdtd} methods. They method discretise the wave equation in both time and spatial domains, using a grid to simulate how waves propagate through a medium. \acrshort{fdtd} methods are widely used in engineering and physics to model sound propagation in complex environments and to study the effects of diffraction and absorption or to model natural phenomena outside the sound domain \citep{teixeira2023finite}.
These techniques cna model wave phenomena with high accuracy and can account for varying medium properties but at the cost of high computational resource requirements. In large environments, such computational costs can limit their application for certain tasks.
...
Principle: Similar to FEM, but uses high-degree polynomials for the basis functions, which can achieve high accuracy with fewer elements under certain conditions.
Application: Used in scenarios where precision is crucial, such as in the design of concert halls and critical listening environments.
Strengths: Combines the flexibility of FEM with the accuracy of spectral methods.
Limitations: Like FEM, SEM can be computationally demanding and requires significant expertise to implement effectively.
Comparison with Geometrical Acoustics
Accuracy at Low Frequencies: Unlike geometrical methods, wave-based methods remain accurate even as the wavelength approaches or exceeds other characteristic dimensions in the problem.
