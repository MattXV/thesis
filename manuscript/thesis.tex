\documentclass{resources/template}
\usepackage{graphicx, parskip, appendix, float, geometry, algorithm, algpseudocode, bigints, multirow}
\usepackage{url, amsmath, amssymb, amsfonts, fancybox, listings, pdfpages, subcaption, rotating, booktabs, ragged2e}
\usepackage{textcomp, xcolor, hyphenat, capt-of, mathtools, natbib, pdflscape, tabularx, array, siunitx, epigraph}
\usepackage[nodayofweek]{datetime}
\renewcommand\tabularxcolumn[1]{m{#1}}


\newcommand*{\urlprefix}{Available from: }
\newcommand*{\urldateprefix}{Accessed }
\bibliographystyle{bath}
\usepackage[font=small]{caption}
\renewcommand*\familydefault{\rmdefault}
\usepackage[T1]{fontenc}
\usepackage[pagebackref=false,pdffitwindow=true]{hyperref}

\geometry{
 a4paper,
 total={150mm,240mm},
 left=30mm,
 top=30mm,
}

% Table row labelling
\newcounter{rowcntr}[table]
\renewcommand{\therowcntr}{\arabic{rowcntr}}
% A new columntype to apply automatic stepping
\newcolumntype{N}{>{\refstepcounter{rowcntr}\therowcntr}c}
% Reset the rowcntr counter at each new tabular
\AtBeginEnvironment{tabular}{\setcounter{rowcntr}{0}}
% X column with ragged right
\newcolumntype{Y}{>{\raggedright\arraybackslash}X}

% glossary options - thanks Sean <3
\usepackage[acronym, nonumberlist, nomain, toc, style=alttree]{glossaries}
\newglossary[mlg]{maths}{mni}{mno}{Mathematical Notation}
\glssetwidest[0]{MUSHRAAAA}
\renewcommand{\glstreenamefmt}{}
\makenoidxglossaries

% glossary entries
\newacronym{aar}{AAR}{Audio Augmented Reality}
\newacronym{dft}{DFT}{Discrete Fourier Transform}
\newacronym{stft}{STFT}{Short-Time Fourier Transform}
\newacronym{dtft}{DTFT}{Discrete-Time Fourier Transform}
\newacronym{fft}{FFT}{Fast Fourier Transform}
\newacronym{ir}{IR}{Impulse Response}
\newacronym{fir}{FIR}{Finite Impulse Response}
\newacronym{iir}{IIR}{Infinte Impulse Response}
\newacronym{rir}{RIR}{Room Impulse Response}
\newacronym{jnd}{JND}{Just-Noticeable Difference}
\newacronym{lod}{LoD}{Level of Detail}
\newacronym{brir}{BRIR}{Binarual Room Impulse Response}
\newacronym{hrtf}{HRTF}{Head-Related Transfer Function}
\newacronym{hrir}{HRIR}{Head-Related Impulse Response}
\newacronym{api}{API}{Application Programming Interface}
\newacronym{naf}{NAF}{Neural Acoustic Field}
\newacronym{dsp}{DSP}{Digital Signal Processing}
\newacronym{mse}{MSE}{Mean Squared Error}
\newacronym{ar}{AR}{Augmented Reality}
\newacronym{xr}{XR}{Extended Reality}
\newacronym{vr}{VR}{Virtual Reality}
\newacronym{mr}{MR}{Mixed Reality}
\newacronym{hmd}{HMD}{Head-Mounted Display}
\newacronym{ve}{VE}{Virtual Environment}
\newacronym{ga}{GA}{Geometrical Acoustics}
\newacronym{ism}{ISM}{Image-Source Model}
\newacronym{fdtd}{FDTD}{Finite-Difference Time-Domain}
\newacronym{cnn}{CNN}{Convolutional Neural Network}
\newacronym{dnn}{DNN}{Deep Neural Network}
\newacronym{nn}{NN}{Neural Network}
\newacronym{rl}{RL}{Reinforcement Learning}
\newacronym{tpu}{TPU}{Tensor Processing Unit}
\newacronym{gpu}{GPU}{Graphics Processing Unit}
\newacronym{gan}{GAN}{Generative Adversarial Network}
\newacronym{bvh}{BVH}{Bounding Volume Hierarchy}
\newacronym{bsp}{BSP}{Binary Space Partitioning}
\newacronym{brdf}{BRDF}{Bidirectional Reflectance Distribution Function}
\newacronym{erb}{ERB}{Equivalent Rectangular Bandwidth}
\newacronym{has}{HAS}{Human Hearing System}
\newacronym{itd}{ITD}{Interaural Time Difference}
\newacronym{ild}{ILD}{Interaural Level Difference}
\newacronym{aabb}{AABB}{Axis-Aligned Bounding Box}

%% MATH SYMBOLS
\DeclarePairedDelimiter\floor{\lfloor}{\rfloor}
\DeclareMathAlphabet{\pazocal}{OMS}{zplm}{m}{n}
\SetMathAlphabet\pazocal{bold}{OMS}{zplm}{bx}{n}
%% -------------------------------------------------------

%% Scientific Units
\sisetup{scientific-notation=true}

\newcommand{\authorName}{Mattia Colombo}
\newcommand{\reportTitle}{Acoustic Information Retrieval for Interactive Sound Rendering in Virtual Environments}
\newcommand{\degreeAward}{Research to PhD in Computing} 

\hypersetup{
    pdftitle    = {\reportTitle},
    pdfauthor   = {\authorName},
    pdfsubject  = {Computing},
    pdfkeywords = {Sound Rendering, Audio Augmented Reality, Psychoacoustics},
    colorlinks  = true, anchorcolor = blue, filecolor = blue, urlcolor = blue,
    linkcolor   = blue,    %NOTE: change (blue) to (colIdentifier) to have links within the document in Black
    citecolor   = blue,    %NOTE: change (blue) to (colIdentifier) to have citation links within the document in Black
}

\definecolor{colBackGrnd}{rgb}{1,1,0.8}
\definecolor{colKeys}{rgb}{0,0,1}
\definecolor{colIdentifier}{rgb}{0,0,0}
\definecolor{colComments}{rgb}{0,.5,0}
\definecolor{colString}{rgb}{0,0,1}
\definecolor{colWhite}{rgb}{1,1,1}

\newcommand{\MyHookSign}{\hbox{\ensuremath\hookleftarrow}}

\newtheorem{Theorem}{Theorem}
\newtheorem{Proposition}[Theorem]{Proposition}
\newtheorem{Lemma}[Theorem]{Lemma}
\newtheorem{Proof}[Theorem]{Proof}
\newtheorem{Remark}[Theorem]{Remark}
\newtheorem{Claim}[Theorem]{Claim}
\newtheorem{Example}[Theorem]{Example}
\newtheorem{Definition}[Theorem]{Definition}

\lstset{%
    float=H,
    basicstyle=\ttfamily\footnotesize,
    identifierstyle=\color{colIdentifier},
    keywordstyle=\color{colIdentifier}, %
    stringstyle=\color{colIdentifier},
    commentstyle=\color{colIdentifier}, %
    columns=flexible,
    tabsize=2,
    frame=single,
    extendedchars=true, %
    showspaces=false,
    showstringspaces=false,
    numbers=left, %
    numberstyle=\footnotesize,
    breaklines=true,
    prebreak={\space\MyHookSign},
    language=C++,
    backgroundcolor=\color{colBackGrnd},
    breakautoindent=true, %
    captionpos=b%
} %\hypersetup{colorlinks=true, citecolor=\color{colIdentifier}}

\DeclareGraphicsExtensions{.jpg,.png,.gif,.pdf,.eps}
\graphicspath{{1_intro/images}
              {2_background/images}
              {3_lit-review/images}
              {4_materials/images}
              {4_materials/images/cog-preliminary}
              {5_rendering/images}
              {6_AR-pipeline/images}
              {7_evaluation/images}
              {8_conclusions/images}
}

% \graphicspath for tables 
\makeatletter
\def\input@path{{1_intro/tables}
              {2_background/tables}
              {3_lit-review/tables}
              {4_materials/tables}
              {5_rendering/tables}
              {6_AR-pipeline/tables}
              {7_evaluation/tables}
              {8_conclusions/tables}
}
\makeatother

\begin{document}
% Front Cover
\title{\huge{\reportTitle}}
\author{\authorName}
\degreetitle{\degreeAward} % Replace with appropriate degree
\rpttype{PhD}    % Replace PhD / MSc  / BSc.
\principaladviser{Dr Carlo Harvey, Dr Maite Frutos-Pascual, Dr Alan Do\l hasz}

% Abstract, Preface, ToC
\include{abstract}
\listofalgorithms%NOTE: Will generate a list of Algorithms in the Table of Contents Section
% \lstlistoflistings%NOTE: Will generate a list of Program Listings in the Table of Contents Section

\printnoidxglossary{}
\cleardoublepage{}
\pagenumbering{arabic}\setcounter{page}{1}
\sloppy
% Chapters
\include{1_intro/introduction}           % ch:introduction
\include{2_background/background}        % ch:background
\include{3_lit-review/lit-review}        % ch:lit-review
\include{4_materials/materials}          % ch:materials 
\include{5_rendering/acoustic-rendering} % ch:acoustic-rendering
\include{6_AR-pipeline/ar-pipeline}      % ch:ar-pipeline
\include{7_evaluation/evaluation}        % ch:evaluation
\include{8_conclusions/conclusions}      % ch:conclusion

\footnotesize
\addcontentsline{toc}{chapter}{Bibliography}
\bibliography{thesis}
\normalsize

\appendix
\include{appendix}

% \chapter{Publications}
% \includepdf[pages=-, offset=0 0, fitpaper=true]{publications/1.pdf}
% \includepdf[pages=-, offset=0 0, fitpaper=true]{publications/2.pdf}
% \includepdf[pages=-, offset=0 0, fitpaper=true]{publications/3.pdf}
% \includepdf[pages=-, offset=0 0, fitpaper=true]{publications/4.pdf}
\end{document}